...
public class LoginManager : MonoBehaviour
{
    ...
    private void OnButtonLoginClick()
    {
        ...
        if (account == "")
        {
            messageText.text = "Ingrese su nombre de cuenta.";
            EnableButtons();
            return;
        }
        if (password == "")
        {
            messageText.text = "Por favor, introduzca su contraseña.";
            EnableButtons();
            return;
        }
        if (account.Length < 2)
        {
            messageText.text = "La longitud del nombre de la cuenta es demasiado corta.";
            EnableButtons();
            return;
        }
        if (password.Length < 2)
        {
            messageText.text = "La longitud de la contraseña es demasiado corta.";
            EnableButtons();
            return;
        }
        ...
        while (authenticating)
        {
            ...
            {
                case 0:
                    messageText.text = "La cuenta no existe.";
                    authenticating = false;
                    break;

                case 1:
                    messageText.text = "La cuenta está prohibida.";
                    authenticating = false;
                    break;
    ...
    {
        ConfirmDialog.Instance.PlayerConfirm("Está seguro que quiere salir del juego? ", 1);
    }
}
